...
class AdvancedAnalytics:
        ...
        for i, z_score in enumerate(z_scores):
            if z_score > 2:
                anomalies.append({
                    ...
                    "anomaly_type": (
                        "extremely_negative"
                        if data_points[i]["sentiment"] < mean
                        else "extremely_positive"
                    ),
                })
    ...
    def sector_clustering(self):
        ...
        if len(sector_features) < 3:
            return {
                "clusters": [],
                "total_sectors": len(sector_features),
                "message": "Insufficient sectors for clustering",
            }
        ...
        # KMeans clustering
        n_clusters = min(3, len(sector_names))  # up to 3 clusters
    ...
    def correlation_analysis(self):
        ...
        for (sector1, sector2), data in sector_pairs.items():
            # lowered threshold from 3 -> 2 so UI has data sooner
            if data["count"] >= 2:
    ...
    def velocity_analysis(self):
        ...
        for sector, dates in sector_timeline.items():
            ...
            if len(sorted_dates) >= 2:
                ...
                previous_avg = (
                    np.mean(dates[sorted_dates[-2]]) if len(sorted_dates) > 1 else recent_avg
                )
                ...
                velocities.append({
                    ...
                    "trend": (
                        "accelerating"
                        if velocity > 0.1
                        else ("decelerating" if velocity < -0.1 else "stable")
                    ),
